...
import 'package:flutter_pkgscan_new/constants/config.dart';
...
class ManifestService {
  ...
  Future<void> addManifest(BuildContext context, String name, String description, IconData icon) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        showSnackBar(
            context: context,
            message: data['message'] ?? 'Manifest added successfully!');
      } else {
        // Hata mesajını göster
        showSnackBar(
            context: context, message: data['message'] ?? 'An error occurred.');
      }
    } catch (e) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          showSnackBar(
              context: context, message: 'Unexpected response format.');
          return [];
        }
      } else {
        // Hata mesajını göster
  ...
  void addManifestToLocalStorage(BuildContext context, String name, String description) {
    try {
      ...
      // Başarı mesajı
      showSnackBar(
          context: context, message: TextConstants.manifestAddedSuccessfully);
    } catch (e) {
      // Hata mesajı
  ...
  void deleteManifestFromLocalStorage(BuildContext context, String id, Box<Manifest> manifestBox) {
    try {
      int indexToDelete = -1;
      for (int i = 0; i < manifestBox.length; i++) {
        if (manifestBox.getAt(i)!.id == id) {
          indexToDelete = i;
          break;
        }
      }
      ...
      if (indexToDelete != -1) {
        manifestBox.deleteAt(indexToDelete);
        showSnackBar(context: context, message: TextConstants.manifestDeletedSuccessfully);
      } else {
        showSnackBar(context: context, message: TextConstants.noManifestWithThisID);
      }
    } catch (e) {
  ...
  void editManifestInLocalStorage(BuildContext context, id, String newName, String newDescription, Box<Manifest> manifestBox) {
    try {
      int indexToEdit = -1;
      for (int i = 0; i < manifestBox.length; i++) {
        if (manifestBox.getAt(i)!.id == id) {
      ...
      if (indexToEdit != -1) {
        final Manifest updatedManifest = Manifest(
          id: id,
          name: newName,
          description: newDescription,
          entities: manifestBox.getAt(indexToEdit)!.entities,
        );

        manifestBox.putAt(indexToEdit, updatedManifest);

        showSnackBar(context: context, message: TextConstants.manifestUpdatedSuccessfully);
      } else {
        showSnackBar(context: context, message: TextConstants.noManifestWithThisID);
      }
    } catch (e) {
