...
class RecordDataService {
  ...
  Future<Map<String, dynamic>?> fetchRecordData(
    ...
    if (data != null && data.isNotEmpty) {
      ...
      RecordHelpers.initializeControllers(controllers, oldValues, record['data']);
      RecordHelpers.initializeDynamicBooleans(dynamicBooleans, oldBooleanValues, record['data']);
  ...
  void prepareCombinedList(Map<String, dynamic> data, List<Map<String, String>> combinedList) {
    ...
    for (int i = 0; i < imageKeys.length; i++) {
      ...
      String title = i < titleKeys.length ? data[titleKeys[i]]?.toString() ?? '' : '';
      String msrp = i < msrpKeys.length ? data[msrpKeys[i]]?.toString() ?? '' : '';
  ...
  List<String> getSortedKeysStartingWith(Map<String, dynamic> data, String prefix) {
